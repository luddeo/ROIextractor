...
for(t_sample in names(project)) {
  ...
  for(t_file in roi_image_files) {
    ...
    for(t_line in 1:length(t_lines)) {
      ...
      out_image[tt_line:(tt_line+image_height-1),t_image_line | tt_image_line,1:3] <- 0
      out_image[tt_line:(tt_line+image_height-1),t_image_line | tt_image_line,4] <- 1
      if(sum(t_image_line) > 0) { # if pixels in the line
        print(paste("line:",t_line))
        out_image[tt_line:(tt_line+image_height-1),t_image_line,3] <- 1
      }
      if(sum(tt_image_line) > 0) { # if pixels in the line
        print(paste("line:",t_line))
        out_image[tt_line:(tt_line+image_height-1),tt_image_line,2] <- 1
